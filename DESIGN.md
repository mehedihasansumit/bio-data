...
- **Notice:** a 12px `ink-muted` line beneath the trigger and above the panel border, present only once the draft is non-empty. It names the risk and the two verbs that answer it — *"Saved on this device only. Clear your browser or change phones and it's gone — copy it or download a file to keep it."* A `<p>` outside the button, referenced by `aria-describedby`, so it is announced as a description rather than swallowed into the control's name.
- **Panel intro:** a matching 12px line inside the panel, carrying the half the notice does not — *"Moving to a new phone or computer? Download a file here, then use **Open a file** there to carry on where you left off."* The notice states a risk; this states a capability, and names the round trip because "Open a file" lives on the device the person has not opened yet. The two never repeat each other: risk outside the collapse, capability inside it.
...
- **Second touchpoint:** after the first print, the hint under the preview swaps from *"Printing? Choose **Save as PDF**…"* to *"That saves a PDF — a picture of the biodata. The copy you can still edit is only in this browser,"* followed by a `paper-emerald` secondary, **Back up this biodata**. One slot, two jobs, in the order they arise. The button does not describe the panel's location — it switches off the mobile preview, opens the disclosure, focuses its trigger, and scrolls it to centre (`behavior: "auto"` under `prefers-reduced-motion`). The disclosure's open state therefore lives in `BuilderClient`, not in `DataTransfer`.

**The Two Touchpoints Rule.** The disclosure sits exactly 70px below Next and is the last element on the page, so anyone walking the form with Previous/Next passes it once per section — six times. It reaches nobody who navigates by the tab strip and never scrolls to the bottom, and no placement *at the bottom* ever will. Print is the second touchpoint because it is the only action every person takes, it lives in a header that needs no scrolling, and it is the last screen before the tab closes. A saved PDF feels like a saved biodata; that is the moment to say it is not one.

**The Closed But Not Silent Rule.** A disclosure may hide its controls; it may never hide its reason to exist. The explanation of what backup is once lived *inside* the panel, so the only person who needed it — someone who did not already know the feature existed — was the one person who never opened it. Whatever sentence justifies a collapsed control belongs outside the collapse.

Its counterweight sits in the sequential-nav row: the autosave status reads **"Saved"**, not "Saved to this device". That region is `aria-live` and re-announces on every pause in typing, so it reports the event and nothing else. The standing fact — that this device holds the only copy — is not an event, and stating it there both spammed screen readers and oversold: "Saved to this device" reads as *your work is safe*, which is the single best reason someone would never go looking for a backup. Reassurance and caveat are different jobs; the live region takes the first, the notice takes the second.

On the landing page this appears once, in the FAQ, as reassurance rather than instruction — no button names. Per the three-modes split above, the landing page persuades; naming four controls on a screen the reader has not opened yet teaches nothing and goes stale the moment the panel moves.
...
**The rule binds the browser's dialog too.** `beforeunload` is the one interruption this product does not draw, and its wording — "Changes you made may not be saved" — cannot be changed: every engine ignores custom strings, and `preventDefault()` is the whole API. So the only thing that can be made honest about it is *when* it fires. It used to fire whenever the form had content, which was false on every reload: the draft is autosaved and `readDraft` restores it, so nothing was at risk and the browser said otherwise anyway. It now fires only when `saveState === "failed"` — quota, which a large photo reliably causes — the one state where storage does not match the screen.

Removing the warning from the ordinary path made the 600ms autosave debounce load-bearing, so the draft is now also flushed synchronously on `visibilitychange` → hidden and on `pagehide`. `visibilitychange` is the one that fires on mobile, where a backgrounded tab can be discarded without `beforeunload` ever running. Silence on reload is only honest if the reload genuinely costs nothing.
